...
export default function CreatorPage() {
  ...
  const renderLoading = () => {
    return (
      <div
        style={{ minHeight: "70vh", textAlign: "center", position: "relative" }}
      >
        <span
          style={{
            position: "absolute",
  ...
  const renderNotSubscribed = () => {
          ...
          .map((item, index) => {
            return (
              <div className="relative pb-6 flex justify-between text-base text-gray-500 font-semibold bg-white col-span-3 p-6 mx-4 rounded-xl border border-gray-50 flex flex-col space-y-6 mb-6">
                <div className="col-span-1 rounded-xlborder  border-gray-50 flex flex-col space-y-6 mb-6">
                ...
                <Link
                  ...
                  to="/post/view"
                  state={{ post: item, creator: selectedCreator }}
                >
                  <svg
        ...
        <>
          <div className="bg-white col-span-3 p-6 mx-4 rounded-xl border border-gray-50 flex flex-col space-y-6 mb-6">
            <p className="text-2xl text-gray-500 font-semibold">
              Subscribe to see {selectedCreator.name} contents
            </p>
          </div>
    ...
    return (
      <div className="bg-white col-span-3 p-6 mx-4 rounded-xl border border-gray-50 flex flex-col space-y-6 mb-6">
        <p className="text-2xl text-gray-500 font-semibold">
          Wait for {selectedCreator.name} first content!
        </p>
      </div>
    );
  };
  ...
  const renderSubscriptionArea = () => {
    ...
    if (selectedSubscription) {
      subscriptionEndString = MyUtil.timeConverter(
        selectedSubscription.subscriptionEnd.toNumber()
      );
      ...
      return (
        <div className="container col-span-1">
          <div className="col-span-1 bg-white p-6 mx-6 rounded-xl border border-gray-50 flex flex-col space-y-6 mb-6">
            <div className="grid">
              <span className="text-xs text-gray-500 font-semibold mb-2 uppercase">
                Support {selectedCreator.name}
              </span>
              <span className="text-sm text-gray-800 font-regular mb-4">
                Connect your wallet to subscribe and see the creator contents.
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="container col-span-1">
          <div className="col-span-1 bg-white p-6 mx-6 rounded-xl border border-gray-50 flex flex-col space-y-6 mb-6">
            <div className="grid">
              ...
              <span className="text-sm text-gray-800 font-regular mb-4">
                Your subscription cost is{" "}
                {MyUtil.convertPriceInSol(
                  selectedCreator.monthPrice.toNumber()
                )}{" "}
                SOL.
              </span>
              <Link
                to={"/creator/account"}
                className="px-4 py-2 text-sm bg-indigo-700 text-white rounded uppercase tracking-wider text-center font-semibold hover:bg-indigo-600 hover:text-indigo-100"
              >
                Click to modify
              </Link>
            </div>
    ...
    if (selectedSubscription) {
      const subscriptionText = subscriptionValid
        ? "Your subscription is valid until: " + subscriptionEndString
      ...
      return (
        <div className="container col-span-1">
          <div className="col-span-1 bg-white p-6 rounded-xl border border-gray-50 flex flex-col space-y-6 mb-6">
            <div className="grid">
              <span className="text-xs text-gray-500 font-semibold mb-2 uppercase">
                You are {!subscriptionValid ? "no more" : ""} subscribed to{" "}
              ...
              {!subscriptionValid && (
                <span className="text-sm text-gray-800 font-regular mb-4">
                  Subscribe now and get 30 days access to all content from this
                  creator
                </span>
              )}
              <a
                ...
                onClick={() =>
                  subscriptionValid
                    ? unsubscribe(selectedCreator)
                    : subscribe(selectedCreator)
                }
                className="px-4 py-2 text-sm bg-indigo-700 text-white rounded uppercase tracking-wider text-center font-semibold hover:bg-indigo-600 hover:text-indigo-100"
              >
                {subscriptionValid
      ...
      <div className="container col-span-1">
        <div className="col-span-1 bg-white p-6 rounded-xl border border-gray-50 flex flex-col space-y-6 mb-6">
          <div className="grid">
            ...
            <a
              href="#"
              onClick={() => subscribe(selectedCreator)}
              className="px-4 py-2 text-sm bg-indigo-700 text-white rounded uppercase tracking-wider text-center font-semibold hover:bg-indigo-600 hover:text-indigo-100"
            >
              {subscriptionValid
                ? "UNSUBSCRIBE"
  ...
  const renderCreatorPage = () => {
    return (
      <div>
        <main
          className="w-screen h-80 bg-indigo-600"
          style={{
            backgroundImage: "url(" + selectedCreator.cover + ")",
        ...
        <main>
          ...
          <div className="mb-6" style={{ marginTop: "-50px" }}>
            <div className="w-full flex flex-col items-center mb-6">
              <div className="h-32 w-32 bg-zinc-400 rounded-full bg-white flex items-center justify-center">
                {selectedCreator.image ? (
                  <img
                    src={selectedCreator.image}
                    className="h-32 w-32 bg-gray-500 border-none rounded-full"
                    alt=""
                  />
                ) : (
                  <>
              ...
              <div
                className="pb-2.5 pl-2.5 pr-2.5"
                style={{ display: "inline-block" }}
              >
                <h1 className="text-3xl font-bold text-center text-gray-800">
                  {selectedCreator.name}
                </h1>
                <h2 className="text-base text-gray-600 font-regular text-center">
                  {selectedCreator.bio}
                </h2>
              </div>
            </div>
        ...
        <main
          className="container mx-w-6xl mx-auto py-4"
    ...
    <body className="relative antialiased bg-gray-100">
      ...
      <div>
        <div className="w-full flex flex-col items-center mb-4 mt-6">
          <div className="mb-3">
            <p className="px-4 text-sm font-semibold text-center text-gray-600">
              Powered by
            </p>
          </div>
          <div>
            <a href="https://solfans.xyz/" target="_blank" rel="noreferrer">
              <img
                src={solfansLogo}
                alt="solfans"
                className="h-16 w-32 border-none"
              />
            </a>
          </div>
        </div>
